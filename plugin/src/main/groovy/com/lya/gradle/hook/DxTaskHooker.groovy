...
class DxTaskHooker extends GradleTaskHooker<TransformTask> {
    ...
    DxTaskHooker(Project project, ApkVariant apkVariant) {
        ...
        task.inputs.files.each { input ->
            if(input.directory) {
                input.eachFileRecurse { file ->
                    if (file.directory && file.path.endsWith(extension.packagePath)) {
                        ...
                        File pkgDir = new File(unzipJarDir, extension.packagePath)
                        if (pkgDir.exists()) {
    ...
    boolean recompileSplitR(File pkgDir) {
        ...
        if(RClassFiles?.length) {
            ...
            String baseDir = pkgDir.path - "${File.separator}${extension.packagePath}"

            project.ant.javac(
                srcdir: extension.splitRJavaFile.parentFile,
